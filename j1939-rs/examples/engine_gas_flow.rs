...
fn main() {
    ...
    println!("Raw Data: {:02X?}", &j1939_msg.data[..j1939_msg.length as usize]);

    // Decode the message back
    let decoded = EngineGasFlowRate::unmarshall(&j1939_msg).unwrap();

    println!("\n=== Decoded Values ===");
    println!("EGR1 Mass Flow Rate: {:.2} kg/h", decoded.egr1_mass_flow_rate);
    println!("EGR1 Error Status: {}", decoded.egr1_error_status);
    println!("Intake Manifold Temp: {}°C", (decoded.intake_manifold_1_temp as i16) - 40);
    println!("EGR1 Temperature: {}°C", (decoded.egr1_temperature as i16) - 40);
    println!("EGR1 Diff Pressure: {:.2} kPa", decoded.egr1_diff_pressure);
    println!("Crankcase Vent Flow: {:.1} L/min", decoded.crankcase_vent_flow_rate);
    println!("EGR1 Control: {:.1}%", decoded.egr1_control);

    // Verify roundtrip accuracy
    println!("\n=== Roundtrip Verification ===");
    let tolerance = 0.1;
    let egr_flow_ok = (decoded.egr1_mass_flow_rate - gas_flow.egr1_mass_flow_rate).abs() < tolerance;
    let diff_pressure_ok = (decoded.egr1_diff_pressure - gas_flow.egr1_diff_pressure).abs() < tolerance;
    let vent_flow_ok = (decoded.crankcase_vent_flow_rate - gas_flow.crankcase_vent_flow_rate).abs() < tolerance;
    let control_ok = (decoded.egr1_control - gas_flow.egr1_control).abs() < tolerance;

    println!("EGR1 Flow Rate: {} (diff: {:.3})",
             if egr_flow_ok { "✓" } else { "✗" },
             (decoded.egr1_mass_flow_rate - gas_flow.egr1_mass_flow_rate).abs());
    println!("EGR1 Error Status: {}",
             if decoded.egr1_error_status == gas_flow.egr1_error_status { "✓" } else { "✗" });
    println!("Intake Temp: {}",
             if decoded.intake_manifold_1_temp == gas_flow.intake_manifold_1_temp { "✓" } else { "✗" });
    println!("EGR1 Temperature: {}",
             if decoded.egr1_temperature == gas_flow.egr1_temperature { "✓" } else { "✗" });
    println!("EGR1 Diff Pressure: {} (diff: {:.3})",
             if diff_pressure_ok { "✓" } else { "✗" },
             (decoded.egr1_diff_pressure - gas_flow.egr1_diff_pressure).abs());
    println!("Crankcase Vent Flow: {} (diff: {:.3})",
             if vent_flow_ok { "✓" } else { "✗" },
             (decoded.crankcase_vent_flow_rate - gas_flow.crankcase_vent_flow_rate).abs());
    println!("EGR1 Control: {} (diff: {:.3})",
             if control_ok { "✓" } else { "✗" },
             (decoded.egr1_control - gas_flow.egr1_control).abs());

    if egr_flow_ok && diff_pressure_ok && vent_flow_ok && control_ok &&
